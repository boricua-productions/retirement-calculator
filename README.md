...
Two entry modes are available via a radio toggle at the top of this section:

- **Simple (bundled totals)** — enter `Base Monthly` and `Minimum Monthly` directly as single JPY scalars (legacy path, unchanged).
- **Detailed (by category)** *(V8.1)* — build the budget from named line items grouped into Essential and Discretional categories. The engine scalars are derived automatically from the category sums. An amber banner in this mode reminds you that NHI and resident tax are NOT entered here.

| Field | What it is for |
|-------|----------------|
| **Base Monthly** | Desired household spending before NHI, resident tax, and Nenkin contribution add-ons. In Detailed mode this is derived from the sum of all category effective monthly amounts. |
| **Minimum Monthly** | Reduced spending floor used when the defensive waterfall cannot fund base spending. In Detailed mode this is derived from the Essential category sum plus the configured buffer. |
...
*(V8.1)* The section also shows a **Location** read-out drawn from the global Japan Location and an **Apply NHI rates for selected city** button. When clicked, the nine rate/cap fields are populated for the selected municipality and a provenance badge ("✓ Authoritative" or "⚠ Estimate") appears. A non-modal blue nudge is shown if the Japan Location changes after rates have been applied; it does not auto-overwrite your values.
...
**Toggle:** `Project Estate / Inheritance Tax` (default: off). When on, the engine runs the estate calculation at end-of-horizon (or at a user-set death date) and displays results in the Overview tab under "Wealth Transferred to Heirs."

| Field | What it is for |
|-------|----------------|
| **Death Date** | Optional date for the estate event. Leave blank to use the simulation end date. |
| **Spouse Death Date** | Optional; informational. When present, the spousal ½ deduction (配偶者の税額軽減) is automatically applied. |
| **Heirs** | List of heirs (name, birth date, relationship: Spouse / Child / Other). Adding a Spouse heir applies the spousal ½ deduction. |
| **Lifetime Gifting Optimiser** | When on, suggests annual gifting amounts using the ¥1.1M Japan 暦年贈与 exclusion and the US §2503(b) $19k exclusion, and estimates the resulting reduction in Sōzoku-zei. *Rough guidance — not legal advice.* |

**How the computation works:**

1. **Japan Sōzoku-zei** — Basic exclusion = ¥30M + ¥6M × heir count. The taxable amount is divided among heirs using statutory shares (spouse ½, children split the rest). Each heir's share is taxed on the NTA bracket schedule (10% to 55%). The spousal ½ deduction is applied when a spouse is present.

2. **US Estate Tax** — Federal exemption: ~$13.6M pre-2026, ~$7M from 2026 onward (TCJA sunset), both inflation-indexed at 2.8%/yr. The rate above the exemption is a flat 40%.

3. **US-Japan Treaty Credit** — Per Article 6 of the 2004 US-Japan Estate Tax Treaty, the US credits Japan inheritance tax proportionate to Japan-situs assets. For a Japan-domiciled US citizen, this typically covers most of the Japan tax bill.

4. **Net to Heirs** — Gross estate minus all estate taxes (Japan Sōzoku-zei + net US estate tax after treaty credit), shown in both JPY and USD.

The Overview tab shows the full breakdown: gross estate, each tax bill (as ¥ / $ and as % of estate), treaty credit applied, and the final net amount transferred to heirs.
...
| **V7.11.1** | Non-blocking Yahoo Finance auto-fetch (background threads + live timers); removed editable Rebalance Date field, replaced with a read-only schedule summary and amber warnings for pre-retirement rebalance dates. |
| **V7.11** | Gradual pre-retirement buffer accumulation — War Chest and Bridge Fund each independently ramp over a configurable 12–36 month window to avoid a large capital-gains event at the retirement transition. |
| **V7.10.1** | Master `war_chest_enabled` / `bridge_fund_enabled` toggles; "Already Set Aside" pre-funded fields so the model only liquidates the gap (Target − Pre-funded) at retirement. |
| **V7.10** | Long-Term Care Insurance (介護保険 / Kaigo Hoken) — age-65+ municipal premium via 9-tier income brackets, smooth NHI transition at 65, and optional care-cost scenarios (None / Low / Medium / High). |
| **V7.9.9** | Cryptocurrency tax engine — Japan miscellaneous-income treatment (up to 55%) vs US LTCG/STCG; `AssetClass::Crypto` with IRC §1091 wash-sale exemption per IRS Notice 2014-21. |
| **V7.9.8** | Correlated Monte Carlo — Cholesky decomposition for historical US-equity / USD-JPY correlation (ρ = −0.40); produces ~28% narrower P10–P90 bands than independent paths; user-configurable correlation matrix with nearest-PSD correction. |
| **V7.9.7** | Estate planning — Japan Sōzoku-zei (8-bracket, spousal ½ deduction), US estate tax with 2026 TCJA sunset, US-Japan Treaty pro-rata credit, and lifetime gifting optimiser (暦年贈与 + §2503(b)). |
| **V7.9** | PFIC §1296 MTM phantom income with §1296(d) loss carry-forward and dual-currency drift cross-check; Real Estate mortgage amortization, rental income routing, and HELOC draw (Tier 7.5). |
| **V7.8.2** | `shock_ordering` enum (`depreciate_then_reprice` / `reprice_then_depreciate` / `simultaneous`) makes dual recession+FX shock sequencing deterministic; `pre_shock_net_worth_jpy` / `post_shock_net_worth_jpy` audit fields on `AnnualSnapshot`. |
| **V7.8.1** | Month-precision dependent phase-outs — fractional NHI per-capita `num_insured`, VA add-on cliff at child's 18th birthday, RSU `num_deps` from December-31 snapshot, `next_12_months_income_jpy` rolling look-ahead. |
| **V7.8** | `SpouseProfile` enum drives IRS filing status — NRA-MFS Roth phase-out (IRC §408A), §6013(g) income pooling, HoH fallback; spouse Japan income fields added to UI and FTC general basket. |
| **V7.7.2** | RSU sell-to-cover death spiral — three-tier cascade (Bridge Fund → War Chest → Tier 8 liquidation) when a recession drops share price below the combined US+Japan tax bill at vest; unpaid liability accumulated and displayed as a red banner. |
| **V7.7.1** | Per-account rebalance strategy (`AccountRebalanceStrategy`) with `migrate_on_retirement`; per-account `us_tax_advantaged` / `japan_tax_advantaged` flags; Japan working-year income tax (所得税) with N−1 salary/RSU hand-off. |
| **V7.7** | Master toggles — `enable_education_savings` suppresses Tier 2.5 accumulation; `enable_gift_sink` disables Tier 9 December gift drain; both default `true` for backward compatibility. |
| **V7.6** | Component-based return profile per asset (`cap_growth`, `dividend_yield`, `interest_yield`, `cap_gains_dist`, `roc`, `expense_ratio`); §904 passive/general basket FTC split; ROC basis reduction across FIFO lots. |
| **V7.5** | PFIC §1296 MTM routing; Japan cap-loss carry-forward (IT Act Art. 37-12-2); Tier 9 Gift Sink with §2503(b) Form 709 flagging; Exit Tax Monitor (Art. 60-2); Ninki Keizoku NHI; stochastic FX in Monte Carlo; TLH (§1091 wash-sale aware). |
| **V7.4** | Jido Teate accrual drift fix; 4-month preemptive buffer restocking for Mode B (Dynamic regime). |
| **V7.3** | Tier 0.5 Jido Teate (child allowance); Tier 2.5 Education Fund; dual withdrawal regimes (Shielded / Dynamic); quarterly dividend cadence defaults per ticker. |
| **V7.2** | US-Japan Treaty Articles 17 & 18 (SS/FERS routing); DC/iDeCo ¥/万口 price model; RSU cliff-accumulation math; FX spread penalty clamping. |
| **V7.1** | Defensive JPY-first spending waterfall with FX spread penalty; lumpy dividend scheduling by calendar month; `dividend_currency` field. |
| **V7.0** | Japan-resident cost basis (`avg_purchase_price_jpy`); highest-JPY-basis-first liquidation ordering. |
| **V6.4 – V6.6** | Family demographics (SSDI, VA child rider, spouse age-gating); marriage toggle + spouse SS/Nenkin; FX drift step-cadence logic. |
| **V5.5 – V6.3** | Target-state rebalancing; Marco Polo Monte Carlo engine; auto-fetch ticker pricing; FEIE/FTC pipeline split and §904/§911 mathematical hardening. |
---
...
| `expenses_detailed_mode` *(V8.1)* | Detailed Mode Toggle | `bool` | `false` | When `true`, the UI derives `base_monthly_expenses_jpy` and `min_monthly_expenses_jpy` from the category sums rather than from direct-entry fields. The two scalars are still written to JSON on every save so the engine is unaffected. |
| `expense_categories` *(V8.1)* | Expense Categories | `Vec<ExpenseCategory>` | `[]` | Array of named budget line items. Each entry has `name` (string), `kind` (`"essential"` or `"discretional"`), `amount_jpy` (f64), `frequency_months` (u32, default 1), optional `end_date` (YYYY-MM-DD), and optional `note`. Essential categories drive both base and minimum spending; Discretional categories drive base only. Categories whose names match the NHI/住民税 deny-list are stripped on save. |
| `min_expense_buffer_jpy` *(V8.1)* | Minimum Buffer (Fixed JPY) | `f64` | `0` | Fixed JPY/month added to the essentials sum when computing `min_monthly_expenses_jpy`. Ignored when `min_expense_buffer_pct > 0`. |
| `min_expense_buffer_pct` *(V8.1)* | Minimum Buffer (% of Essentials) | `f64` | `0` | Percentage of the essentials sum added to minimum (e.g. `0.10` = +10%). Takes precedence over the fixed-JPY buffer when non-zero. |
...
| `tests/detailed_expenses.rs` *(V8.1)* | 10 | Legacy file loads with defaults (1), round-trip detailed mode (2), Essential stop-rule emitted with correct dates/flags (3), Discretional stop-rule skips floor (4), already-expired category excluded (5), engine equivalence simple vs detailed (6), NHI reserved-name is stripped by loader (7), `looks_like_reserved_category` deny-list (8), `effective_monthly_jpy` amortises correctly (9), zero-frequency clamp (10) |
...
- **Location row** *(V8.1)*: read-only "Location: Prefecture / City" label drawn from the global Japan Location section; **Apply NHI rates for selected city** button (disabled when no location is set) populates all nine rate/cap fields and stamps a provenance badge — "✓ Authoritative" (Sagamihara) or "⚠ Estimate" (all other municipalities). A non-modal blue nudge appears when the Japan Location changes after rates have been applied; it does not auto-overwrite user values.
...
V8.0.1 is a non-logic cleanup pass (let-chain refactors, eta-reduced closures, unused-import
removal, Windows build documentation);
V8.1 adds detailed expense entry by category — users now build their monthly budget from named
...
### V8.1 — Detailed Expense Entry by Category

**Motivation:** Users previously entered a single `base_monthly_expenses_jpy` scalar, making
mid-simulation expense changes (child ageing out, loan payoff, sabbatical spending) require manual
stop-rule authoring. Detailed mode externalises this bookkeeping into the UI.

**Key design decisions:**

1. **Engine isolation.** The cashflow engine is unchanged. It still consumes `base_expense_jpy`
   / `min_expense_jpy` scalars. Categories are a loader/UI concern. The engine gains only two new
   `ExpenseRule` flags (`apply_to_floor: bool`, `inflate: bool`) with fully backward-compatible
   defaults (`true` / `false`).

2. **Synthetic stop-rules.** When a category has an `end_date` inside the simulation window,
   the loader generates a negative `ExpenseRule` named `CategoryStop:<name>`. The rule carries
   `inflate: true` (so the stop tracks the same CPI factor as the base scalars) and
   `apply_to_floor: matches!(kind, Essential)` (Discretional stops do not reduce the minimum floor).

3. **NHI guard.** `looks_like_reserved_category` (public, in `src/models/expense.rs`) blocks
   the most common double-counting confusions: `nhi`, `national health`, `kokumin kenko`,
   `juminzei`, `jumin-zei`, `resident tax`, 国民健康保険, 住民税.

4. **Non-destructive toggle.** `expenses_detailed_mode` and `expense_categories` are always
   persisted in JSON. Toggling the mode does not destroy the other mode's data.

5. **NHI provenance model.** `NhiRateProvenance` (`Authoritative` | `Estimate`) in
   `src/engine/tax/japan_regions.rs` badges the source quality of the loaded NHI rates.
   The location-change nudge (`nhi_rates_applied_for` session cache) notifies without
   auto-overwriting user values.
